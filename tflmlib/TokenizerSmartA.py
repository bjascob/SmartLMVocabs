...
class TokenizerSmartA(Tokenizer):
    def __init__(self, dict_fn):
        super(TokenizerSmartA, self).__init__()
    ...
    def tokenizeSentence(self, sent):
        ...
        i = 0
        while i < sent_len:
            # Always use named-entity as the token if available
            if sent['ner'][i] != 'O':
                tokens.append(sent['ner'][i])
                i = self.skipMultiTokens(i, sent['ner'])
            # Use POS token for proper-nouns, cardinal numbers, foreign words and symbols
            elif sent['pos'][i].startswith('NNP') or sent['pos'][i] == 'CD' \
                    or sent['pos'][i] == 'FW' or sent['pos'][i] == 'SYM':
        ...
        if tokens[-1] != '.':
            tokens.append('.')
        # Note that most code puts this at the end of a sentence for some
        # reason, even when there's a period
        # tokens.append('<eos>')
        return tokens
